...
function appendmap(id,countryname){
  d3.json("data/uscountywithname.json", function(error, us) {
    ...
    var chartDiv = document.getElementById(id.replace("#",""));
    ...
    var g = svg.append("g").attr("transform", "scale(" + 0.3 + ")"+"translate(10,200)");;
...
d3.csv("data/countryDegree.csv", function(error, data) {
  ...
  var chartDiv = document.getElementById("degreeChart");
...
// header text
  ...
  .enter().append("text")
    ...
    .text(function(d){return d})
      .attr("x", function(d,i) {
        if (i == 0) return 0; 
        else if (i == 1) return (xPadding*2)*i-30
        else return (xPadding*2)*i-20
      })
      .attr("y", -10)
...
//advanced
  ...
  svg.selectAll("rect[style = 'fill: steelblue;']")
      .on("mouseover", function () {
        ...
        hoverOverLogEvent.classname = "hoverover";
        hoverOverLogEvent.currenttime = Date.now();
        hoverOverLogEvent.currenttext = "degreeChart";
        visOverInteraction.push(hoverOverLogEvent);
        console.log(hoverOverLogEvent)
      })
      .on("mouseout", function () {
        var hoverOutLogEvent = {};
          hoverOutLogEvent.currenttime = Date.now();
          hoverOutLogEvent.currenttext = "degreeChart";
          visOutInteraction.push(hoverOutLogEvent);
      });
...
d3.csv("data/countrySalary.csv", function(error, data) {
  if (error) throw error;
  var chartDiv = document.getElementById("salaryChart");
  ...
  svg.selectAll(".salarybar")
    ...
    .enter().append("rect")
      ...
      .on("mouseover", function () {
        ...
        hoverOverLogEvent.classname = "hoverover";
        hoverOverLogEvent.currenttime = Date.now();
        hoverOverLogEvent.currenttext = "salaryChart";
        visOverInteraction.push(hoverOverLogEvent);
      })
      .on("mouseout", function () {
        var hoverOutLogEvent = {};
          hoverOutLogEvent.currenttime = Date.now();
          hoverOutLogEvent.currenttext = "salaryChart";
          visOutInteraction.push(hoverOutLogEvent);
      });
...
d3.csv("data/countryCitizen.csv", function(error, data) {
  if (error) throw error;
  var chartDiv = document.getElementById("citizenChart");
  var margin = {top: 20, right: 10, bottom: 30, left: 60},
    width = chartDiv.offsetWidth - margin.left - margin.right,
    height = 300 - margin.top - margin.bottom;
  ...
  svg.selectAll(".citizenbar")
    ...
    .enter().append("rect")
      ...
      .on("mouseover", function () {
        ...
        hoverOverLogEvent.classname = "hoverover";
        hoverOverLogEvent.currenttime = Date.now();
        hoverOverLogEvent.currenttext = "citizenChart";
        visOverInteraction.push(hoverOverLogEvent);
      })
      .on("mouseout", function () {
        var hoverOutLogEvent = {};
          hoverOutLogEvent.currenttime = Date.now();
          hoverOutLogEvent.currenttext = "citizenChart";
          visOutInteraction.push(hoverOutLogEvent);
      });
...
d3.csv("data/countryArrival.csv", function(error, data) {
  if (error) throw error;
  var chartDiv = document.getElementById("arrivalChart");
  var margin = {top: 10, right: 20, bottom: 22, left: 35},
    width = chartDiv.offsetWidth - margin.left - margin.right,
  ...
  g.append("g")
    ...
    .enter().append("rect")
      ...
      .on("mouseover", function () {
        ...
        hoverOverLogEvent.classname = "hoverover";
        hoverOverLogEvent.currenttime = Date.now();
        hoverOverLogEvent.currenttext = "arrivalChart";
        visOverInteraction.push(hoverOverLogEvent);
      })
      .on("mouseout", function () {
        var hoverOutLogEvent = {};
          hoverOutLogEvent.currenttime = Date.now();
          hoverOutLogEvent.currenttext = "arrivalChart";
          visOutInteraction.push(hoverOutLogEvent);
      });
  ...
  g.selectAll("axistime")
    ...
    .enter().append("text")
      ...
      .attr("x", function(d,i){return (x1.bandwidth()+7)*i})
      .attr("y", function(d,i){
